...
**Amendment (2026-09-10):** Round-4 flow successor (`aerobeat-web-assembly-8tz4`) deleted the Flow Grid ruleset; the sole visible Flow label is exactly `Flow`, backed by `flow_colliders_v1`.

The default `aero-prototype-selector` development presenter exposes exactly five choices: `Flow` (the sole Flow mode, preserving profile ID `flow` and ruleset `flow_colliders_v1`) and the four exact Boxing ruleset/recipe combinations, while retaining its visual, ruleset, and converter identity/version/hash telemetry. Flow intentionally selects the renderer-owned `flow` presentation outside this package. The scoped product presenter derives independent controls from the exact `selectedProfileId`: Gameplay is `Flow` (`flow_colliders_v1`), `Boxing Lanes` (`boxing_semantic_track_v1`), or `Boxing Grid` (`boxing_spatial_grid_v1`); Boxing also exposes Conversion as `Balanced Height` (`row_family_balanced_height_v1`) or `Source Height` (`cut_family_source_height_v1`). Invalid or omitted selection falls back to Flow. Flow exposes Obstacles and never exposes or mutates Boxing conversion. All four Boxing combinations remain selectable without a preferred winner. Compact presentation may visually suppress the outer Gameplay heading, but keeps the Boxing `Conversion` legend visible so the second native radio group remains understandable to sighted users.

Scoped Gameplay emits `gameplay-mode-select` with the exact scalar payload `{ rulesetId }`, including exact `flow_colliders_v1` when Flow is chosen. Scoped Conversion emits `boxing-conversion-select` with the exact scalar payload `{ recipeId }`. Assembly combines each intent with bounded current/retained mode and conversion state, resolves an existing exact variant, and projects that `selectedProfileId` back in the next snapshot. Because Flow has no conversion selection, assembly must retain its last Boxing conversion choice or establish product policy outside this presenter before resolving a Flow-to-Boxing intent; the UI does not choose a conversion winner. No combined variant, package, profile, or service object crosses the UI boundary. Default development select/import-request/export/reset behavior remains unchanged; the presenter does not mutate profiles or regenerate content.
