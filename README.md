...
- `aero-visual-test-transport`: compact bottom Visual Test transport. Its exact immutable state is `{ active, playing, currentMs, durationMs }`; it is hidden when `active` is false, clamps time to a bounded duration, and presents Play/Pause, a native range, and right-side total-minute `mm:ss` timecode. It emits empty `visual-test-play`/`visual-test-pause` intents and exact scalar `visual-test-seek` `{ milliseconds }`. Beginning a scrub requests Pause once; release never infers resume. Assembly owns audio, generation binding, seek coalescing, timeline synchronization and rendering.
