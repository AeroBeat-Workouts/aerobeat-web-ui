...
- `aero-flow-hud`, `aero-boxing-track-hud`, `aero-boxing-spatial-hud`: mode-specific presentation only; the existing Flow presenter now uses exact visible `Flow Grid` copy while preserving its public element name.
...
- `aero-prototype-selector`: the default, omitted-`scope` mode exposes exactly six development choices: `Flow Grid`, experimental `Flow Colliders`, and all four internal Boxing ruleset/recipe combinations, alongside bounded `profileClasses` states for `live_visual`, `between_run_ruleset`, and `converter_regeneration`. Flow Grid retains profile ID `flow` and ruleset `flow_grid_v2`; Flow Colliders uses profile ID `flow-colliders` and ruleset `flow_colliders_v1`. Each state consumes the exact public seven-field tuning identity (`schema`, `version`, ID/version, bare SHA-256, class, regeneration truth) and may separately label the host choice as experimental; visual choices apply immediately, scoring choices are disabled with a live reason during play/countdown and enabled between runs, and converter telemetry distinguishes selected/applied/pending hashes plus regeneration truth. Product embeds may instead set `scope="gameplay"` for exactly `Flow Grid`, `Flow Colliders`, `Boxing Lanes`, and `Boxing Grid`; a Boxing selection also shows a separate `Conversion` native-radio group containing `Balanced Height` and `Source Height`. The exact `selectedProfileId` snapshot deterministically selects the corresponding ruleset and recipe, while invalid or omitted snapshots fall back to Flow Grid and never auto-select Flow Colliders. Both Flow modes omit Boxing conversion and expose the existing Flow obstacle choices. Scoped views omit development metadata and management controls. Import/export/reset and profile-selection intents retain scalar identity fields only; the host resolves and owns profile bundles.

Intent details are `{ type, payload }`. Payloads contain scalar IDs/query values only. Scoped Gameplay emits `gameplay-mode-select` with exactly `{ rulesetId }`; the value is one of `flow_grid_v2`, `flow_colliders_v1`, `boxing_semantic_track_v1`, or `boxing_spatial_grid_v1`. The Flow modes' Obstacles group emits `flow-obstacle-mode-select` with exactly `{ mode }` for `default`, `no_obstacles`, or `obstacle_visual_only`. The Boxing-only Conversion group emits `boxing-conversion-select` with exactly `{ recipeId }`; the value is `row_family_balanced_height_v1` or `cut_family_source_height_v1`. Assembly combines either scalar intent with its bounded current/retained mode and conversion state and returns the resolved exact `selectedProfileId` snapshot. Flow Grid and Flow Colliders have no conversion selection, so assembly must retain its last Boxing conversion choice (or establish product policy outside this presenter) before resolving a Flow-to-Boxing mode intent; this UI does not choose a conversion winner. The UI never emits a combined variant, package, or profile object. Compact BeatSaver Download carries exact `mapId`/`versionHash`; default development import retains `difficultyId`. Compact downloaded-song selection carries `collectionId`, downloaded Difficulty carries exact `collectionId`/`packageId`, Preview/Export carry `packageId`, and Delete carries `collectionId`. Raw `File`, ZIP/audio bytes, media objects, screenshots, provider DTOs and service objects never leave UI. External snapshots are narrowed into immutable JSON-like records without executing accessors. Visual Test transport state rejects accessors, prototypes, symbols, extra fields and non-finite/negative values; no package, media, service or authored-event object crosses its boundary.
