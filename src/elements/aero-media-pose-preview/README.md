...
`aero-media-pose-preview` composes a video-owned media surface with the shared PlayCanvas landmark-overlay renderer.

The element does not request camera permission, run CV inference, or draw landmarks itself. Consumers inject or use the public `@aerobeat/web-video` facade for live camera, loaded video, or replay media attachment, then pass normalized pose frames to `setPoseFrame(...)`. The element retains one canvas and one `AeroPlayCanvasRenderer`, delegates exact CSS/DPR sizing and landmark/skeleton drawing to `@aerobeat/web-renderer`, and owns every render request; PlayCanvas never starts a second animation loop.
...
- `setPoseFrame(poseFrame)` updates the normalized pose frame drawn by PlayCanvas.
